  ...
  User.findOne({
    ...
    .then((user) => {
      ...
      if (user) {
        ...
        var _msghtml = fs.readFileSync(
          "./app/templates/forgetpasswordotp.html",
          "utf8"
        );
        const message = {
          ...
          html: _msghtml
            .replace("@FoodieUser", "Foodie")
            .replace("@Email", user.user_login)
            .replace("@CODEHERE", otp),
        };
        sendEmail(message);
    ...
    .then((user) => {
      if (user) {
        ...
        if (_data.type == "verifyemail") {
          var _msghtml = fs.readFileSync(
            "./app/templates/emailverificationotp.html",
            "utf8"
          );
          const message = {
            from: authConfig.smtp.sender,
        ...
        context.done(
          null,
          send_response(200, { message: "OTP sent to your registered Email" })
        );
      } else {
